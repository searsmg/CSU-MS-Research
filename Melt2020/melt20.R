### Script to calculate Melt 20 data ###
...
setwd("C:/Users/sears/Documents/Research/Snow_Hydro_Research/Thesis/Data/Air Temp/For R/2020")

##########################################################################
#### PROCESSING ####

#temp, elevation, and elevation band data
temp_elev_20 <- read.csv(file="AirT_elev20_noOUT.csv", header=TRUE) %>%
  mutate(Datetime = mdy_hm(Datetime))

RH_dewpt <- read.csv(file="RH_dewpt_all.csv", header=TRUE) %>%
  mutate(Datetime = mdy_hm(Datetime))

all20 <- merge(temp_elev_20, RH_dewpt, by=c("Datetime", "ID"))
...
minute(bandT_long$Datetime) <- 0
minute(bandRH_long$Datetime) <- 0
